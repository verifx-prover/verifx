...
import be.vub.kdeporre.verifx.Compiler.Plugins.ScalaCompilerPlugin.{ClassTable, ScalaExp}
import be.vub.kdeporre.verifx.Compiler.{IR, Plugin, ProjectCompiler}
...
object ScalaCompilerPlugin {
  ...
  val veriFxOps = "be.vub.kdeporre.verifx.Runtime.Implicits"
}
...
class ScalaCompilerPlugin extends Plugin[ScalaExp, ScalaExp, ClassTable] {
  ...
  def startFile(maybePath: Option[String]): State[ClassTable, ScalaExp] = State.pure {
    import be.vub.kdeporre.verifx.Utilities.Utils.RichString
