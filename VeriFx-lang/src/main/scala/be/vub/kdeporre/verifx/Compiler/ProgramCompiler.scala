...
import be.vub.kdeporre.verifx.Analysis.Proofs
import be.vub.kdeporre.verifx.Compiler.Plugins.Z3CompilerPlugin
import be.vub.kdeporre.verifx.Compiler.ProgramCompiler.FileName
import be.vub.kdeporre.verifx.Analysis.Proofs.{ProofName, ProofResult}
import be.vub.kdeporre.verifx.Compiler.Types.{Classes, Proof}
import be.vub.kdeporre.verifx.ParseError
...
object ProgramCompiler {
  ...
  private val imports = "import be.vub.kdeporre.verifx.Compiler.IR._"
...
case class ProgramCompiler(file: List[Stat], clazzes: Classes = Classes(), proofs: Set[Proof] = Set(), proofIndex: Map[ProofName, Proof] = Map(), filePath: Option[FileName] = None) {
  ...
  private def compilationState[C, M <: C, S](plugin: Plugin[C, M, S]): State[S, List[C]] = {
    ...
    val compileBody = irProgram traverse {
      irStat => {
        val ir = toolbox.eval(toolbox.parse(irStat)).asInstanceOf[be.vub.kdeporre.verifx.Compiler.IR.IR]
        compiler.compile(ir)
      }
    }
